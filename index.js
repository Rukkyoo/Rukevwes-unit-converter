...
inputBtn.onclick = function () {
  if (textBox.value === "") {
    ...
    volumeCalc.textContent = `${textBox.value} liters = ${gallonsCalc} gallons | ${textBox.value} feet = ${litersCalc} liters`;
    massCalc.textContent = `${textBox.value} kilograms = ${poundsCalc} pounds | ${textBox.value} feet = ${kilogramsCalc} kilograms`;
  }
};

function baseState() {
  lengthCalc.textContent = `0 meters = 0 feet | 0 feet = 0 meters`;
  volumeCalc.textContent = `0 meters = 0 feet | 0 feet = 0 meters`;
  massCalc.textContent = `0 meters = 0 feet | 0 feet = 0 meters`;
}
